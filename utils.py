...
def structure_to_unit(structure):
    ...
    labels =  [s._label for s in sites_to_keep]
    unit = Molecule(species=species, coords=coords, labels=labels)
    unit  = unit.get_boxed_structure(structure.lattice.a*4, structure.lattice.b*4, structure.lattice.c*4)
    return unit
